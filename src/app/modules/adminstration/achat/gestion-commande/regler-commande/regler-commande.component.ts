...
@Component({
  selector: 'app-regler-commande',
  templateUrl: './regler-commande.component.html',
  styleUrls: ['./regler-commande.component.scss']
})
export class ReglerCommandeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    onReglerCommande() {
      this.commandeService.reglerCommande(this.infoDaTa.id).subscribe({
        next: (data) => {
          this.utilitisService.response(data, (d: any) => {
            if (data.status == 200) {
              console.log("======commande régler avec Success", d);
              this.showNotification("success");
            } else {
              console.log("======la commande n'a pas pu être régler", d);
